...
class Empaquetado {
  ...
  async guardarPaquetePagado(factura) {
    ...
    if (!userRef.idPaquetePago) {
      ...
      userRef.idPaquetePago = await db
...
async function consultarPendientes(e) {
  ...
  let reference = db.collection(nameCollectionDb).orderBy("timeline");
