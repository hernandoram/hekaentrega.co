...
async function response(datos) {
  ...
  let div_principal = document.createElement("DIV"),
    ...
    head = crearNodo(
      `<h4 class="text-center mb-3">Seleccione transportadora</h4>`
    ),
...
async function detallesTransportadoras(data) {
  ...
  for (let transp in transportadoras) {
    ...
    if (
      ["ENVIA", "INTERRAPIDISMO", "SERVIENTREGA", "COORDINADORA"].includes(transp) &&
    ...
    const encabezado = `<li class="list-group-item list-group-item-action shadow-sm mb-2 border border-${
...
function cambiarPreciosOficinasPorTransportadora(target, cotizacion, oficinas) {
  ...
  const porcentaje_oficina = oficina.precios
    ? oficina.precios.porcentaje_comsion
    : 10;
...
class CalcularCostoDeEnvio {
  ...
  get flete() {
    ...
    console.log("FLETE SERVI" + this.total_flete);
  ...
  async cotizarInter(dane_ciudadR, dane_ciudadD) {
    ...
    let mensajeria = res.filter(
      (d) => d.IdServicio === 3 || d.IdServicio === 6
    );
  ...
  intoCoord(cotizacion) {
    ...
    this.sobreflete = this.convencional
      ? 0
      : Math.max(this.valor * 0.0265, 4300);
...
async function enviar_firestore(datos) {
  ...
  firestore
    .collection("infoHeka")
    .doc("heka_id")
    .update({ id: firebase.firestore.FieldValue.increment(1) });
