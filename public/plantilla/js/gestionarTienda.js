...
$("#logo-portada-tienda").on("mouseenter", () =>
  $("#actualizar-logo-portada").show("fast")
);
$("#logo-portada-tienda").on("mouseleave", () =>
  $("#actualizar-logo-portada").hide("fast")
);
...
Dropzone.options.imagenesProducto = {
  ...
  maxFiles: 10,
  dictDefaultMessage:
    "Arreastre o Cargue las imágenes a mostrar<br><i class='fa fa-upload fa-2x'></i>",
...
class VentanaCrearProducto {
  ...
  appendAttributes() {
    ...
    definicion.innerHTML =
      "<h6 class='ml-2'><small>Puede que tu producto tenga uno, varios o ningún atributo o característica. Elige los atributos de tu producto y sus respectivas variaciones.</small></h6>";
  ...
  appendVariants() {
    ...
    let definicion = document.createElement("div");
    definicion.innerHTML =
      "<h6 class='ml-2'><small>Cada variación tiene un precio, código de almacenamiento y una cantidad en inventario.</small></h6>";
    definicion.classList.add("text-muted", "border-left-secondary");
  ...
  fillVariants() {
    ...
    for (let atrib of variantes) {
      ...
      tr.innerHTML = `
                ...
                <td><input class="form-control" data-campo-stock="cod" value="${arrAttr.join(
                  "-"
                )}" readonly/></td>
  ...
  saveVariants() {
    ...
    $("[data-campo-stock]").each((i, v) => {
      ...
      if (typeof this.stock[counter].detalles != "object")
        this.stock[counter] = new Object();
      this.stock[counter].detalles[campo] = isNaN(parseInt(v.value))
        ? v.value
        : parseInt(v.value);
    });
  }
    ...
    data.stock.forEach((stock, i) => {
      ...
      for (let attr in stock) {
        $(".atributo, .campo").each((i, btn) => {
          ...
          if (
            (btn.value == stock[attr] || btn.value == attr) &&
  ...
  fillImagesPreloaded(urls) {
    ...
    for (let urlInfo of urls) {
      ...
      if (urls.length < 6) {
        action.innerHTML =
          "<i class='fa fa-trash d-md-none'></i><span class='d-none d-md-block text-truncate'>Eliminar</span>";
      } else {
      ...
      action.setAttribute(
        "class",
        "btn btn-danger badge badge-pill float-right eliminar-imagen"
      );
  ...
  deleteImage(urlInfo, urls) {
    ...
    storageRef
      .child(urlInfo.fileName)
      .delete()
      .catch(() => {
        console.log(
          "El documento .../" + this.id + "/" + urlInfo.fileName + " no existe"
        );
      });

    //y a su vez, actualizo el banco de imágenes del producto
    const productoRef = doc(collection(tiendaDoc, "productos"), this.id);

    updateDoc(productoRef, { imagesUrl: urls }).then(() => {
  ...
  async deleteProducto() {
    let producto = doc(collection(tiendaDoc, "productos"), this.id);
  ...
  async createProduct() {
    ...
    let docRef = collection(tiendaDoc, "productos");
    let docId;
    ...
    this.modal.find("[required]").each((i, e) => {
      ...
      if (!e.checkValidity()) {
        ...
        $(e)
          .parent()
          .append(
            "<div class='d-flex text-danger mensaje-error'><i class='fa fa-exclamation-circle'></i><small class='ml-1'>Este campo también es importante</small></div>"
          );
        invalid = true;
...
async function fillProducts() {
  ...
  await getDocs(collection(tiendaDoc, "productos")).then((querySnapshot) => {
    list.html("");
                    ...
                    <div class="row">
                        ...
                        <div class="col d-none d-md-block">
                            ...
                            <p class="d-none d-md-block">${
                              data.stock[0].detalles.cod
                            }</p>
                        </div>
                        <div class="col">
                            <small class="text-muted d-md-none">Precio: <br/>
                            $${convertirMiles(
                              data.stock[0].detalles.precio
                            )}</small>
                            <p class="d-none d-md-block">$${convertirMiles(
                              data.stock[0].detalles.precio
                            )}</p>
                        </div>
                        <div class="col">
                            <small class="text-muted d-md-none">Cantidad: <br/>${
                              data.stock[0].detalles.cantidad
                            } Unids.</small>
                            <p class="d-none d-md-block">${
                              data.stock[0].detalles.cantidad
                            } Unidades.</p>
...
function obtenerLinkTienda(data) {
  ...
  if (!data || !data.tienda) {
    ...
    link.innerHTML =
      "Actualmente la tienda no se encuentra activa. Por favor, llene la información correspondiente para activarla.";
...
async function actualizarTienda() {
  ...
  if (
    (await verificarExistenciaTienda()) ||
    invalid.length ||
    !Object.entries(ciudad).length
  )
    return;
...
function fillPedidos() {
  let fecha_inicio = Date.parse(
      value("fecha_inicio-pedidos").replace(/\-/g, "/")
    ),
    fecha_final =
      Date.parse(value("fecha_final-pedidos").replace(/\-/g, "/")) + 8.64e7;

      const pedidosQuery = query(
        collection(tiendaDoc, "pedidos"),
        orderBy("timeline", "desc"),
        startAt(fecha_final),
        endAt(fecha_inicio)
      );
      
      getDocs(pedidosQuery).then((querySnapshot) => {
      let data = new Array();
      querySnapshot.forEach((doc) => {
        let dato = doc.data();
  ...
  mostrarInfo(target, table) {
    ...
    if (row.child.isShown()) {
      ...
      if (this.infoGuardada[guia]) {
        ...
        const guiasCollection = collection(usuarioDoc, "guias");
        const guiaDoc = doc(guiasCollection, guia);

        getDoc(guiaDoc).then((doc) => {
          if (doc.exists()) {
      ...
      $(target).html(
        "<button class='btn btn-danger btn-circle btn-sm'><i class='fa fa-minus'></i></button>"
      );
...
function cargarLogoPortada() {
  ...
  this.disabled = true;
  this.innerHTML =
    "<span class='spinner-grow spinner-grow-sm'></span> Cargando";
...
function agregarFuncionalidadesTablaPedidos() {
  ...
  $(".selector").click(function () {
    ...
    tr.find(".selector").each((i, el) => {
      if ($(el).hasClass("active")) filters.push($(el).attr("data-filter"));
    });
...
async function calcularCostoEnvioPedidos(e, dt, node, config) {
  ...
  for (let i = 0; i < datas.length; i++) {
    ...
    for (let transp of trasnportadoras) {
      ...
      let cotizacionPC = await new CalcularCostoDeEnvio(
        recaudo,
        "PAGO CONTRAENTREGA",
        peso,
        volumen,
      ...
      let cotizacionC = await new CalcularCostoDeEnvio(
        recaudo,
        "CONVENCIONAL",
        peso,
        volumen,
      ...
      if (!cotizacionPC.empty) {
        if (pedido.sumar_envio)
          cotizacionPC.sumarCostoDeEnvio = cotizacionPC.valor;
...
async function crearGuiasDesdePedido(e, dt, node, config) {
  ...
  Cargador.fire(
    "Creando guías",
    "Estamos generando las guías solicitadas, esto podría demorar unos minutos, por favor espere."
  );
  ...
  for (let i = 0; i < nodos.length; i++) {
    ...
    guia.centro_de_costo =
      storeInfo.centro_de_costo || datos_usuario.centro_de_costo;
    ...
    let cotizacion = await new CalcularCostoDeEnvio(
      recaudo,
      guia.type,
      peso,
      volumen,
    ...
    if (cotizacion.empty) {
      notificacionPorGuia(
        row,
        "Sin resultados por parte de la transportadora seleccionada",
        "exclamation-circle",
        "text-danger"
      );
      continue;
    ...
    if (cotizacion.seguro < cotizacion.costoEnvio) {
      notificacionPorGuia(
        row,
        "El valor del recaudo no debe ser menor al costo del envío.",
  ...
  function notificacionPorGuia(row, mensaje, icon, colorText) {
    $(row).after(
      `<tr><td colspan='10' class='${colorText}'><i class='fa fa-${icon} mr-2'></i>${mensaje}</td></tr>`
    );
  }
