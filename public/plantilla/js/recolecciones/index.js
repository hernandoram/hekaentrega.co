...
async function llenarRecoleccionesPendientes() {
    ...
    // .where("recoleccion_esporadica", "==", true)
...
function formSolicitarRecoleccion(e) {
    ...
    const recoleccionTramitada = recoleccionesPendientes[consulta.idSucursalCliente];
    consulta.id_user = recoleccionTramitada.id_user;
    
    const {ids_heka, numerosGuia} = recoleccionTramitada.guias.reduce((a,b) => {
...
const acciones = {
    solicitarRecoleccion: e => {
        ...
        const m = new CreateModal({
            title: "Solicitud de recolección para: " + datos_recoleccion.centro_de_costo
        });
