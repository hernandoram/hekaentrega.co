...
exports.revisarEstadoFinalizado = (estado) => {
  const listaEstadosFinalizadaCompleta = estadosEntregado.concat(
    estadosAnuladas,
    estadosDevolucion
  );
...
exports.actualizarReferidoPorGuiaEntregada = async (data, nuevosDatos) => {
  ...
  const dataReferido = await referidoRef
    .get()
...
exports.modificarEstadoGuia = (guia) => {
  ...
  if (
    [estados.empacada, estados.generada, estados.novedad].includes(
      estadoActual
    ) &&
    estadoTransportadora
  ) {
  ...
  if (
    seguimiento_finalizado &&
    ![this.estadosGuia.pagada].includes(actualizaciones.estadoActual)
  ) {
  ...
  if (
    oficina &&
    estadosTransportadora[transportadora].entregada.includes(
      estadoTransportadora
    )
  ) {
    ...
    if (!estadoFlexii) actualizaciones.estadoFlexii = estadoBaseFlexi;
  }
...
exports.detectaNovedadEnElHistorialDeEstados = (respuestaMovimientos) => {
  if (
    respuestaMovimientos.estado === "Mov.A" &&
    respuestaMovimientos.guardado
  ) {
...
exports.obtenerEstadosGuiaPorId = async (id_user, id_heka) => {
  return refEstadosGuia(id_user, id_heka)
    .get()
    .then((doc) => (doc.exists ? doc.data() : null));
};

exports.crearOActualizarEstados = async (
  id_user,
  id_heka,
  data,
  dataExistente = false
